...
export type RequestHeaders = RequestControlOptions & Record<string, RequestHeaderValue>
...
export type RouteRequestConfig = Partial<Omit<RequestRuntimeConfig, 'header'>> & {
  url: string | RegExp
  header?: RequestHeaderFactory
}
...
export class RequestError extends Error {
  ...
  constructor(message: string, options: { code?: number; statusCode?: number; response?: unknown } = {}) {
    super(message)
...
function normalizeResponseData<T>(response: ApiResponse<T>): T {
  if (Object.prototype.hasOwnProperty.call(response, 'data')) return response.data as T

  if (Object.prototype.hasOwnProperty.call(response, 'rows') || Object.prototype.hasOwnProperty.call(response, 'total')) {
    return {
      list: response.rows || [],
...
function getErrorMessage(response?: ApiResponse<unknown>) {
  return errorCode[response?.code || 0] || response?.msg || errorCode['default']
}
...
function buildRequestHeader(headers: RequestHeaders | undefined, runtimeConfig: RequestRuntimeConfig, aesKey?: CryptoJS.lib.WordArray) {
  const requestHeaders: Record<string, string> = headers?.isNotToken ? {} : { ...runtimeConfig.header() }
...
export function request<T, D = unknown>(options: RequestOptions<D>): Promise<T> {
  ...
  const requestData = transformRequestData(options.data, Boolean(options.headers?.isEncrypt), aesKey)
  const requestHeader = buildRequestHeader(options.headers, runtimeConfig, aesKey)
    ...
    uni.request({
      ...
      success: (res) => {
        ...
        if (keyStr != null && keyStr != '') {
          ...
          response.data = JSON.parse(decryptWithAes(response?.data as string || '', decryptBase64(aesKey.toString())))
        }
...
export const http = {
  get: <T, D = unknown>(url: string, data?: D, options: Omit<RequestOptions<D>, 'url' | 'method' | 'data'> = {}) => {
  ...
  delete: <T, D = unknown>(url: string, data?: D, options: Omit<RequestOptions<D>, 'url' | 'method' | 'data'> = {}) => {
