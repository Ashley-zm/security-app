...
export async function getDictsByTypes(dictTypes: string[], isHasEmpty = false) {
    try {
        const dicts = await Promise.all(dictTypes.map(type => getDictsApi({ dictType: type })))
...
// 去除请求参数中的空值
export function removeEmptyParams(params: Record<string, string | number | boolean | object | undefined>) {
